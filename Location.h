...
class Location {
private:
	virtual Area processCommand(string input, Player &PC) = 0; // Processes the user's input string to check it against actions unique to the area.  Returns the area the player ends up in afterward.
...
public:
	...
	Area getCommand(string input, Player &PC); // Takes input and uses the virtual processCommand function to process it.  Returns the area the player ends up in after the command is processed.
};
